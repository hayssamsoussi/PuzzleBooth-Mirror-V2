...
class ServerService : IntentService("ServerService") {
    ...
    override fun onHandleIntent(intent: Intent?) {
        ...
        try {
            ...
            Log.e("hhh ServerService", "成功启动server")
            //开启一个死循环，不断接受数据
            while (running) {
                try {
                ...
                Log.e("hhh ServerService", "成功接收信息")
                //解析收到的数据
                receiveAddress = receivePacket.address.toString()
                receiveAddress = receiveAddress.substring(1, receiveAddress.length)
                lastReceiverAddress = receiveAddress
                Log.e("hhh ServerService", "该数据包来自:$receiveAddress")
                //跳过自己发送的数据
            ...
            //关闭DatagramSocket对象
            service.takeIf { service?.isConnected == true }?.close()
        }
        mLocalBroadcastManager.unregisterReceiver(receiver)
        Log.e("hhh ServerService", "server关闭")
    }

    //处理接收的信息
    private fun receiveMsg(receiveMsg: String, address: String){
        Log.e("hhh ServerService", "收到信息：$receiveMsg")
    ...
    inner class ServerReceiver: BroadcastReceiver(){
        override fun onReceive(p0: Context?, intent: Intent?) {
            when (intent?.action){
                STOP_SERVER -> {
                    service.takeIf { it?.isConnected == true }?.close()
                    running = false
                }
            }
        }
